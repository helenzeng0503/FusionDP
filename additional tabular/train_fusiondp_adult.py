...
# ============================================================
# Adult-Income schema (explicit types)
# ============================================================
# Numeric features in the classic Adult dataset documentation:
ADULT_NUMERIC = ["age", "fnlwgt", "education-num", "capital-gain", "capital-loss", "hours-per-week"]

# Categorical features in the classic Adult dataset documentation:
ADULT_CATEGORICAL = ["workclass", "education", "marital-status", "occupation", "relationship", "race", "sex", "native-country"]
...
class TabDataset(Dataset):
    def __init__(self, X: np.ndarray, y: np.ndarray):
        # Expect X contiguous float32 already
        self.X = torch.from_numpy(X)  # zero-copy
...
# --- Representation Consistency Loss (normalized) ---
...
def build_X_y(df: pd.DataFrame, target_col: str) -> Tuple[pd.DataFrame, pd.Series]:
    ...
    # Adult income label mapping if still strings
    if y.dtype == object:
...
def parse_args():
    ...
    p.add_argument("--support_csv", type=str, default="adult_support.csv")
    p.add_argument("--train_csv", type=str, default="adult_train.csv")
    p.add_argument("--train_imputed_csv", type=str, default="adult_train_imputed.csv")
    p.add_argument("--val_csv", type=str, default="adult_val.csv")
    p.add_argument("--test_csv", type=str, default="adult_test.csv")
    p.add_argument("--target_col", type=str, default="income")
    p.add_argument(
        "--sensitive_cols",
        type=str,
        nargs="+",
        default=["sex", "race", "native-country", "marital-status", "age"],
    )
    ...
    p.add_argument("--results_json", type=str, default="results_adult_seed.json")
    return p.parse_args()
...
def main():
    ...
    # Ensure columns match between train and train_imputed (except maybe ordering)
    ...
    # One-hot: fit on TRAIN ONLY (union of real/imp/noise), then apply same columns to val/test
    if args.one_hot:
        # --- split into numeric + categorical (explicit Adult schema) ---
        def split_num_cat(Xdf: pd.DataFrame):
            X_num = Xdf[ADULT_NUMERIC].apply(pd.to_numeric, errors="coerce").fillna(0.0)
            X_cat = Xdf[ADULT_CATEGORICAL].astype(str).fillna("__NA__")
            return X_num, X_cat
        ...
        # imputed + noise views needed for some modes
        Xtr_i_num, Xtr_i_cat = split_num_cat(Xtr_imp)
        Xtr_n_num, Xtr_n_cat = split_num_cat(Xtr_noise)

        # --- one-hot TRAIN categorical only; reindex others to TRAIN columns ---
        Xtr_cat_oh = pd.get_dummies(Xtr_cat, dummy_na=True)
        cat_cols = Xtr_cat_oh.columns

        Xva_cat_oh = pd.get_dummies(Xva_cat, dummy_na=True).reindex(columns=cat_cols, fill_value=0)
        Xte_cat_oh = pd.get_dummies(Xte_cat, dummy_na=True).reindex(columns=cat_cols, fill_value=0)
        Xtr_i_cat_oh = pd.get_dummies(Xtr_i_cat, dummy_na=True).reindex(columns=cat_cols, fill_value=0)
        Xtr_n_cat_oh = pd.get_dummies(Xtr_n_cat, dummy_na=True).reindex(columns=cat_cols, fill_value=0)
        ...
        print("  cat_oh d =", Xtr_cat_oh.shape[1], "  num d =", Xtr_num.shape[1])


    # Standardize: fit on TRAIN REAL, then transform others (keep float32)
    ...
    # Only build imp/noise arrays if needed by mode
    ...
    # Make contiguous float32 (important for from_numpy)
    ...
    if args.mode in ["feature-dp", "fusiondp"]:
        if Xtr_imp_np is None:
            raise ValueError("Mode requires train_imputed features but Xtr_imp_np is None. Did you forget --train_imputed_csv or the file?")
    ...
    for epoch in range(args.epochs):
        ...
        if args.mode == "fusiondp":
            # fusiondp uses global tensors; iterate train_loader only for step count
        ...
        # Evaluate on val + test each epoch (mirrors sepsis-style quick feedback; test is still the main)
