...
# Fixed top-k policies for FEATURES (applied to real columns for ALL splits)
FEATURE_TOPK_POLICY = {
    # high-cardinality
    "native-country": 10,
    "occupation": 20,

    # medium-cardinality (capping helps RAM but usually doesn't hurt much)
    "education": 12,
    "workclass": 8,
    "marital-status": 7,
    "relationship": 6,
...
# ===========================
# 2) Helpers (DO NOT change model logic)
# ===========================
...
def impute_column(
    ...
    if target_col in CAT_TARGETS:
        ...
        y_support = df_support[target_col].astype(float).values
    ...
    # 7) Convert back for categorical + optional eval
    if not is_regression:
        ...
        y_true_num = df_query[target_col].astype(float).values
...
if __name__ == "__main__":
    ...
    print("Fetching Adult (UCI id=2)...")
    ds = fetch_ucirepo(id=2)
    X = ds.data.features.copy()
    y = ds.data.targets.copy()

    # Clean object columns + map "?" to NaN
    for c in X.columns:
        if X[c].dtype == "object":
    ...
    # Attach labels (keep same behavior as before: support has no y)
    for col in y.columns:
        df_train[col] = y.iloc[tr_idx].reset_index(drop=True)[col]
        df_val[col]   = y.iloc[val_idx].reset_index(drop=True)[col]
        df_test[col]  = y.iloc[test_idx].reset_index(drop=True)[col]
    ...
    for target in SENSITIVE_TARGETS:
        ...
        # If this is a capped categorical column, re-apply vocab to guarantee it stays within top-k+OTHER
    ...
    # Save CSVs
    # ---------------------------
    df_support.to_csv("adult_support.csv", index=False)
    df_train.to_csv("adult_train.csv", index=False)
    df_train_imputed.to_csv("adult_train_imputed.csv", index=False)
    df_val.to_csv("adult_val.csv", index=False)
    df_test.to_csv("adult_test.csv", index=False)

    print("\n✅ Saved datasets:")
    print(" - adult_support.csv")
    print(" - adult_train.csv")
    print(" - adult_train_imputed.csv")
    print(" - adult_val.csv")
    print(" - adult_test.csv")
